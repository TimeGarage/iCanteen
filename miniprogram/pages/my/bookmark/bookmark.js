// miniprogram/pages/my/bookmark/bookmark.js
Page({
          ...
          wx.getStorage({
            ...
            success: function (res) {
              ...
              wx.cloud.database().collection('user').where({
                ...
                success: res => {
                  that.setData({
                    bookmarkList: res.data[0].bookmark
                  });
                  console.log(that.data.bookmarkList)
                  var i = 0
                  var myBookmark = that.data.bookmarkList
                  console.log(myBookmark)
                  for (i = 0; i < myBookmark.length; i++) {
                    console.log('[查询] ' + myBookmark[i])
                    wx.cloud.database().collection('discuss').where({
                      _id: myBookmark[i]
                    }).get({
                      success: res => {

                        that.data.discussMenu.push(res.data[0])
